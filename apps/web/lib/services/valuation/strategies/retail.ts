...
export async function buildRetailValuationAnalysis(
  ...
  const valuation = buildStabilizedIncomeValuation(bundle, context, buildRetailValuationConfig());
  ...
  const analysis: UnderwritingAnalysis = {
    ...
    underwritingMemo: '',
    keyRisks: buildStabilizedIncomeKeyRisks(bundle, valuation, retailRiskConfig),
    ddChecklist: buildStabilizedIncomeDdChecklist(retailDdChecklistBase, debtDdChecklistItem),
    assumptions: buildStabilizedIncomeAssumptions(
      'RETAIL',
      valuation,
      bundle.comparableSet?.entries.length ?? 0
    ),
    provenance: buildStabilizedIncomeProvenance(bundle, valuation, retailProvenanceConfig),
