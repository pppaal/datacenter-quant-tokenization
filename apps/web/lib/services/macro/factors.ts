...
function buildFactorPoint(input: Omit<MacroFactorPoint, 'value' | 'isObserved'> & { value: number | null }) {
...
export function buildMacroFactorSnapshot(input: {
  ...
  const debtCost = getSeriesValue(snapshot, 'debt_cost_pct') ?? input.marketSnapshot?.debtCostPct ?? null;
  const discountRate =
  ...
  const vacancy = getSeriesValue(snapshot, 'vacancy_pct') ?? input.marketSnapshot?.vacancyPct ?? null;
  ...
  const rateLevel = [policyRate, debtCost, discountRate].filter((value): value is number => value != null);
  const averageRateLevel =
    rateLevel.length > 0 ? rateLevel.reduce((sum, value) => sum + value, 0) / rateLevel.length : null;

  const rateMomentumCandidates = [
    policyRate != null && previousPolicyRate != null ? (policyRate - previousPolicyRate) * 100 : null,
    debtCost != null && previousDebtCost != null ? (debtCost - previousDebtCost) * 100 : null,
    discountRate != null && previousDiscountRate != null ? (discountRate - previousDiscountRate) * 100 : null
  ].filter((value): value is number => value != null);
  const rateMomentumBps =
    rateMomentumCandidates.length > 0
      ? rateMomentumCandidates.reduce((sum, value) => sum + value, 0) / rateMomentumCandidates.length
      : 0;

  const constructionPressure =
    (constructionCostIndex != null ? constructionCostIndex - 100 : 0) + (inflation != null ? inflation * 4 : 0);
  ...
  const factors: MacroFactorPoint[] = [
    buildFactorPoint({
      ...
      inputs: buildInputsList([inflation != null ? `Inflation ${round(inflation)}%` : 'No current observation'])
    }),
    buildFactorPoint({
      key: 'rate_level',
      label: 'Rate Level',
    ...
    buildFactorPoint({
      ...
      inputs: buildInputsList([creditSpread != null ? `Spread ${round(creditSpread)} bps` : 'No current observation'])
    }),
    buildFactorPoint({
      key: 'liquidity',
      label: 'Liquidity',
      ...
      inputs: buildInputsList([transactionVolume != null ? `Volume index ${round(transactionVolume)}` : 'No current observation'])
    }),
    buildFactorPoint({
      key: 'growth_momentum',
      ...
      commentary:
        rentGrowth == null
          ...
          : rentGrowth >= 2.5
            ...
            : 'Growth is modest.',
      inputs: buildInputsList([rentGrowth != null ? `Rent growth ${round(rentGrowth)}%` : 'No current observation'])
    }),
    buildFactorPoint({
    ...
    buildFactorPoint({
      ...
      inputs: buildInputsList([
        ...
        rentGrowth == null && transactionVolume == null && vacancy == null ? 'No current observation' : null
      ])
    })
  ];
...
export function buildMacroFactorCreateInputs(input: {
  ...
  const observationDate = input.observationDate ?? (snapshot.asOf ? new Date(snapshot.asOf) : input.sourceUpdatedAt);
