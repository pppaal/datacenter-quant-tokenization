      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        {report && a && resolved && cls && macro && pf && rm && dc && capMatrix && md && refi && (
          <div className="space-y-6">
            ...
            {v && (
              <Section title="Investment Verdict (deterministic rubric)">
                ...
                <div className="text-xs text-zinc-500 mb-3 font-mono">
                  Score {v.totalScore}/{v.maxPossibleScore} (normalized{' '}
                  {v.normalizedScore.toFixed(2)}) · target IRR {v.hurdlesUsed.targetLeveredIrrPct}%
                  · floor P10 {v.hurdlesUsed.floorP10IrrPct}% · max Prob(&lt;8%){' '}
                  {(v.hurdlesUsed.maxProbBelow8Pct * 100).toFixed(0)}%
                </div>

                <div className="overflow-x-auto mb-4">
                  <table
                    className="w-full text-sm font-mono"
                    aria-label="Verdict dimension scoring"
                  >
                    <thead>
            ...
            {bid && (
              <Section title="Implied Bid Prices">
                ...
                <div className="mt-3 text-xs text-zinc-500">
                  Positive % = discount below base price. "Base IRR" uses the deterministic
                  pro-forma; MC variants run 400-iteration Monte Carlo per bisection step.
                </div>
              </Section>
            )}
            ...
            {pfx && (
              <Section
                title="4b. Basis / Depreciation / Exit Costs"
                collapsible
                defaultOpen={false}
              >
                <KeyValueRow
                      ...
                      {[
                        {
                          ...
                          pick: (y: (typeof pfYears)[number]) => y.grossPotentialRevenueKrw
                        },
                        {
                          label: '  (–) Downtime Loss',
                          pick: (y: (typeof pfYears)[number]) =>
                            -(y.downtimeLossKrw + y.renewalDowntimeLossKrw)
                        },
                        {
                          label: '  (–) Rent-Free Loss',
                          pick: (y: (typeof pfYears)[number]) =>
                            -(y.rentFreeLossKrw + y.renewalRentFreeLossKrw)
                        },
                        {
                          label: '  Contracted Rent (net)',
                          pick: (y: (typeof pfYears)[number]) => y.contractedRevenueKrw
                        },
                        {
                          label: '  Renewal Rent (net)',
                          pick: (y: (typeof pfYears)[number]) => y.renewalRevenueKrw
                        },
                        {
                          label: '  Residual / Market Rent',
                          pick: (y: (typeof pfYears)[number]) => y.residualRevenueKrw
                        },
                        {
                          label: '  Fixed Recoveries',
                          pick: (y: (typeof pfYears)[number]) => y.fixedRecoveriesKrw
                        },
                        {
                          label: '  Site Recoveries',
                          pick: (y: (typeof pfYears)[number]) => y.siteRecoveriesKrw
                        },
                        {
                          label: '  Utility Pass-Through',
                          pick: (y: (typeof pfYears)[number]) => y.utilityPassThroughRevenueKrw
                        },
                        {
                          label: '  Total Reimbursements (subtotal)',
                          pick: (y: (typeof pfYears)[number]) => y.reimbursementRevenueKrw
                        },
                        {
                          label: 'Total Operating Revenue',
                          pick: (y: (typeof pfYears)[number]) => y.totalOperatingRevenueKrw,
                          bold: true
                        },
                        {
                          label: '  Revenue (alt subtotal)',
                          pick: (y: (typeof pfYears)[number]) => y.revenueKrw
                        },
                        {
                          label: 'Operating Expenses',
                          pick: (y: (typeof pfYears)[number]) => -y.operatingExpenseKrw
                        },
                        {
                          label: '  Power Cost',
                          pick: (y: (typeof pfYears)[number]) => -y.powerCostKrw
                        },
                        {
                          label: '  Site Opex',
                          pick: (y: (typeof pfYears)[number]) => -y.siteOperatingExpenseKrw
                        },
                        {
                        ...
                        },
                        {
                          ...
                          pick: (y: (typeof pfYears)[number]) => -y.maintenanceReserveKrw
                        },
                        {
                          label: 'NOI',
                          pick: (y: (typeof pfYears)[number]) => y.noiKrw,
                          bold: true
                        },
                        {
                          label: 'Tenant Improvement (TI)',
                          pick: (y: (typeof pfYears)[number]) => -y.tenantImprovementKrw
                        },
                        {
                          label: 'Leasing Commission (LC)',
                          pick: (y: (typeof pfYears)[number]) => -y.leasingCommissionKrw
                        },
                        {
                          label: '  Tenant Capital (TI+LC subtotal)',
                          pick: (y: (typeof pfYears)[number]) => -y.tenantCapitalCostKrw
                        },
                        {
                          label: 'Renewal Tenant Capital',
                          pick: (y: (typeof pfYears)[number]) => -y.renewalTenantCapitalCostKrw
                        },
                        {
                          label: 'Fit-Out Cost',
                          pick: (y: (typeof pfYears)[number]) => -y.fitOutCostKrw
                        },
                        {
                          label: 'CFADS (pre-debt)',
                          pick: (y: (typeof pfYears)[number]) => y.cfadsBeforeDebtKrw,
                          bold: true
                        },
                        {
                          label: 'Occupied (kW)',
                          pick: (y: (typeof pfYears)[number]) => y.occupiedKw,
                          isKw: true
                        },
                        {
                          label: 'Contracted (kW)',
                          pick: (y: (typeof pfYears)[number]) => y.contractedKw,
                          isKw: true
                        },
                        {
                          label: 'Residual Occupied (kW)',
                          pick: (y: (typeof pfYears)[number]) => y.residualOccupiedKw,
                          isKw: true
                        },
                        {
                          label: 'Active Renewal Leases',
                          pick: (y: (typeof pfYears)[number]) => y.activeRenewalLeaseCount,
                          isCount: true
                        },
                        {
                          label: 'Wtd Renewal Rate (KRW/kW)',
                          pick: (y: (typeof pfYears)[number]) => y.weightedRenewalRatePerKwKrw,
                          isRatePerKw: true
                        }
                      ].map((row) => (
                  ...
                  >
                    ...
                    <tbody>
                      {[
                        ...
                        {
                          label: 'Debt Draw',
                          pick: (y: (typeof pfYears)[number]) => y.drawAmountKrw
                        },
                        {
                          label: 'Interest',
                          pick: (y: (typeof pfYears)[number]) => -y.interestKrw
                        },
                        {
                          label: 'Principal',
                          pick: (y: (typeof pfYears)[number]) => -y.principalKrw
                        },
                        {
                          label: 'Debt Service (total)',
                          pick: (y: (typeof pfYears)[number]) => -y.debtServiceKrw
                        },
                        {
                          label: 'Ending Debt Balance',
                          pick: (y: (typeof pfYears)[number]) => y.endingDebtBalanceKrw
                        },
                        {
                          label: 'Property Tax',
                          pick: (y: (typeof pfYears)[number]) => -y.propertyTaxKrw
                        },
                        {
                          label: 'Insurance',
                          pick: (y: (typeof pfYears)[number]) => -y.insuranceKrw
                        },
                        {
                          label: 'Management Fee',
                          pick: (y: (typeof pfYears)[number]) => -y.managementFeeKrw
                        },
                        {
                          label: 'Reserve Contribution',
                          pick: (y: (typeof pfYears)[number]) => -y.reserveContributionKrw
                        },
                        {
                          label: 'Corporate Tax',
                          pick: (y: (typeof pfYears)[number]) => -y.corporateTaxKrw
                        },
                        {
                          label: 'After-Tax Distribution',
                          pick: (y: (typeof pfYears)[number]) => y.afterTaxDistributionKrw,
                          bold: true
                        },
                        {
                          label: 'DSCR',
                          pick: (y: (typeof pfYears)[number]) => y.dscr,
                          isRatio: true
                        }
                      ].map((row) => (
            ...
            {pf && rm && pfYears.length > 0 && (
              <Section title="4e. Equity Waterfall" collapsible defaultOpen={false}>
                {(() => {
                  ...
                  const cumulativeDistributions = pfYears.reduce(
                    (s, y) => s + y.afterTaxDistributionKrw,
                    0
                  );
            ...
            {pfx && pfYears.length > 0 && (
              <Section title="4f. Balance Sheet (10-year)" collapsible defaultOpen={false}>
                <div className="overflow-x-auto">
                  <table
                    className="w-full min-w-[720px] text-sm font-mono"
                    aria-label="Annual balance sheet"
                  >
                    <thead>
                      <tr className="text-zinc-400">
                        <th scope="col" className="p-2 text-left">
            ...
            {pfYears.length > 0 && (
              <Section title="4g. Debt Schedule (10-year)" collapsible defaultOpen={false}>
                <div className="overflow-x-auto">
                  <table
                    className="w-full min-w-[720px] text-sm font-mono"
                    aria-label="Annual debt schedule"
                  >
                    <thead>
                      <tr className="text-zinc-400">
                        <th scope="col" className="p-2 text-left">
                    ...
                    <tbody>
                      {(() => {
                        ...
                        const exitCapProxy =
                          pf.terminalValueKrw > 0 ? termNoi / pf.terminalValueKrw : null;
                ...
                <p className="mt-2 text-xs text-zinc-500">
                  Implied value uses a constant exit-cap proxy (terminal NOI / terminal value)
                  applied to each year's NOI. DSCR below 1.15× is flagged in red.
                </p>
              </Section>
            )}
            ...
            {wf && (
              <Section title="4h. GP/LP Promote Waterfall" collapsible defaultOpen={false}>
                ...
                <p className="mt-2 text-xs text-zinc-500">
                  ...
                  Tier 1 returns capital pro-rata; Tier 2 pays the compounded preferred return 100%
                  to LP; Tier 3 catches GP up to the promote split; Tier 4 splits residual profits.
                  GP Promote = GP total minus pro-rata share (the carry earned above passive
                  participation).
                </p>
              </Section>
            )}
            ...
            {mc && (
              <Section title={`5b. Monte Carlo (${mc.iterations} iter, seed ${mc.seed})`}>
                <div className="mb-2 text-xs text-zinc-400">
                  Correlated truncated-normal draws on entry cap / exit cap / rent growth / interest
                  rate via Cholesky decomposition. Joint-stress tails (rates ↑ &amp; caps ↑ &amp;
                  growth ↓) are modeled — P10 reflects co-movement risk.
                </div>
                <table
                  className="w-full text-sm font-mono"
                ...
                <table
                  ...
                  aria-label="Probability equity IRR below target"
                >
                  <thead>
                    <tr className="text-zinc-400">
                ...
                {mc.leveredIrr.histogram && mc.leveredIrr.histogram.length > 0 && (
                  <>
                    ...
                    <div className="flex items-end gap-1 h-24">
                      {mc.leveredIrr.histogram.map((b: any, i: number) => {
                        const maxCount = Math.max(
                          ...mc.leveredIrr.histogram.map((x: any) => x.count)
                        );
                    ...
                    <div className="mt-1 flex justify-between text-xs text-zinc-500 font-mono">
                      <span>
                        {mc.leveredIrr.min !== null ? `${mc.leveredIrr.min.toFixed(1)}%` : ''}
                      </span>
                      <span>
                        {mc.leveredIrr.max !== null ? `${mc.leveredIrr.max.toFixed(1)}%` : ''}
                      </span>
                    </div>
                  </>
                )}
                ...
                <div className="mt-1 text-xs text-zinc-500">
                  Left: assumed (from literature). Right: realized Pearson ρ across{' '}
                  {mc.validIterations} valid iterations (clamping slightly attenuates tails). Green
                  = positive, red = negative co-movement.
                </div>
                ...
                <table className="text-sm font-mono">
                  ...
                  <tbody>
                    {capMatrix.cells.map((row: any[], r: number) => (
                      <tr
                        key={r}
                        className={r === capMatrix.baseRowIndex ? 'bg-indigo-950/30' : ''}
                      >
...
// ---------------------------------------------------------------------------
// Data-quality / provenance panel — surfacing only.
// ---------------------------------------------------------------------------
...
function SourcePill({ tier }: { tier: string }) {
  return (
    <span
