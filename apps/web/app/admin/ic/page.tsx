    ...
    <div className="space-y-8">
      <section className="surface hero-mesh">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Badge>
            {formatNumber(workspace.dashboard.summary.activePacketCount, 0)} active packets
          </Badge>
        </div>
        ...
        <p className="mt-4 max-w-4xl text-base leading-8 text-slate-200">
          IC governance sits between review-ready underwriting and released investment decisions.
          Packet state, meeting schedule, and the latest decisions stay linked to the same approved
          evidence and valuation set.
        </p>
        <div className="mt-6 grid gap-4 md:grid-cols-4">
          <div className="metric-card">
            <div className="fine-print">Scheduled Meetings</div>
            <div className="mt-3 text-4xl font-semibold text-white">
              {formatNumber(workspace.dashboard.summary.scheduledCount, 0)}
            </div>
          </div>
          <div className="metric-card">
            <div className="fine-print">Locked Packets</div>
            <div className="mt-3 text-4xl font-semibold text-white">
              {formatNumber(workspace.dashboard.summary.lockedCount, 0)}
            </div>
          </div>
          <div className="metric-card">
            <div className="fine-print">Released Decisions</div>
            <div className="mt-3 text-4xl font-semibold text-white">
              {formatNumber(workspace.dashboard.summary.releasedCount, 0)}
            </div>
          </div>
          <div className="metric-card">
            <div className="fine-print">Packaging Candidates</div>
            <div className="mt-3 text-4xl font-semibold text-white">
              {formatNumber(workspace.dashboard.summary.candidateCount, 0)}
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mt-5 grid gap-3">
          {workspace.dashboard.actionItems.length > 0 ? (
            ...
            <div className="rounded-[22px] border border-white/10 bg-white/[0.03] p-4 text-sm text-slate-400">
              No committee action items are open. Review-ready assets can be packaged once a
              decision request is defined.
            </div>
          )}
        </div>
      </Card>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
              ...
              workspace.meetings.map((meeting) => (
                ...
                >
                  ...
                  <p className="mt-3 text-sm leading-7 text-slate-300">
                    {meeting.summary ?? 'No meeting summary recorded yet.'}
                  </p>
                  <div className="mt-3 grid gap-2">
                    {meeting.packets.map((packet) => (
                      <div
                        key={packet.id}
                        className="rounded-[18px] border border-white/10 bg-slate-950/35 p-3"
                      >
                        ...
                        <div className="mt-2 text-xs text-slate-400">
                          {packet.asset?.assetCode ?? 'no asset'} /{' '}
                          {packet.valuationRun?.runLabel ?? 'no valuation linked'}
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <Card>
          ...
          <h2 className="mt-2 text-2xl font-semibold text-white">
            Assets ready for packet assembly
          </h2>
          <div className="mt-5 space-y-3">
                ...
                >
                  ...
                  {candidate.diligenceSummary ? (
                    <div className="mt-3 rounded-[18px] border border-white/10 bg-slate-950/35 p-3 text-sm text-slate-300">
                      ...
                      <div className="mt-2">
                        {candidate.diligenceSummary.signedOffCount}/
                        {candidate.diligenceSummary.coreRequiredTypes.length} core lanes signed off
                      </div>
                      <div className="mt-2">
                      ...
                      {candidate.diligenceSummary.blockedCount > 0 ? (
                        <div className="mt-2 text-amber-200">
                          {candidate.diligenceSummary.blockedCount} blocked lane(s) require
                          intervention before packet lock.
                        </div>
                      ) : null}
              ...
              >
                ...
                <p className="mt-3 text-sm leading-7 text-slate-300">
                  {packet.decisionSummary ??
                    packet.followUpSummary ??
                    'No decision note recorded yet.'}
                </p>
                {latestDecision ? (
                ...
                {packet.status !== 'LOCKED' && packet.status !== 'RELEASED' ? (
                  <div className="mt-3 flex flex-wrap items-start justify-between gap-3">
                    ...
                    <CommitteePacketLockButton
                      ...
                      disabledReason={
                        !packet.lockReadiness.canLock
                          ? (packet.lockReadiness.blockers[0] ?? null)
                          : null
                      }
                    />
                  </div>
                ...
                {packet.status === 'LOCKED' ? (
                  <CommitteePacketDecisionForm packetId={packet.id} />
                ) : null}
                {packet.status === 'APPROVED' ||
                packet.status === 'CONDITIONAL' ||
