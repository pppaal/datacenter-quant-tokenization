...
function toneForRealtimeClass(
  status: 'REALTIME' | 'NEAR_REALTIME' | 'RELEASE_BASED' | 'LOW_FREQUENCY'
) {
...
export default async function SourcesPage() {
  ...
  const configuredCount = macroConnectors.filter(
    ...
    <div className="space-y-6">
      <div>
        ...
        <h2 className="mt-2 text-3xl font-semibold text-white">
          Adapter health and cache freshness
        </h2>
        <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-400">
          NASA climate ingestion now tracks POWER climatology and daily near-real-time refresh
          separately from GPM precipitation and FIRMS hotspot overlays.
        </p>
        {canRefreshSources ? (
          <div className="mt-4">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="metric-card">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Some env keys exist, but the connector still cannot run end-to-end.
          </p>
        </div>
        <div className="metric-card">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Best path to replace fallback-heavy market assumptions and static FX conversion with
            live feeds.
          </p>
        </div>
      </div>
      ...
      <Card className="space-y-4">
        <div>
          ...
          <h3 className="mt-2 text-xl font-semibold text-white">
            Batch status, stale assets, and recent source refresh runs
          </h3>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-400">
            Source refresh is now an explicit operator or cron-triggered job. Use this panel to see
            stale adapter coverage, stale assets waiting for enrichment, and the most recent run
            results before research or underwriting relies on them.
          </p>
        </div>
        <div className="grid gap-4 md:grid-cols-3">
          <div className="rounded-[22px] border border-white/10 bg-white/[0.03] p-4">
            ...
            <div className="mt-3 text-2xl font-semibold text-white">
              {refreshHealth.staleThresholdHours}h
            </div>
            <p className="mt-2 text-sm text-slate-400">
              Assets older than this enrichment window enter the refresh queue.
            </p>
          </div>
          <div className="rounded-[22px] border border-white/10 bg-white/[0.03] p-4">
          ...
          <div className="rounded-[22px] border border-white/10 bg-white/[0.03] p-4">
            ...
            <div className="mt-3 text-2xl font-semibold text-white">
              {refreshHealth.assetFreshness.staleCandidates}
            </div>
            <p className="mt-2 text-sm text-slate-400">
              Assets waiting for re-enrichment before the research fabric is fully current.
            </p>
          </div>
        </div>
            ...
            <div className="mt-4 space-y-3">
              {recentRuns.length > 0 ? (
                recentRuns.map((run) => (
                  <div
                    key={run.id}
                    className="rounded-[18px] border border-white/10 bg-slate-950/40 p-4"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        <div className="text-sm font-semibold text-white">
                          {run.triggerType.toLowerCase()} refresh
                        </div>
                ...
                refreshHealth.assetFreshness.staleAssets.map((asset) => (
                  ...
                  >
                    ...
                    <div className="mt-2 text-xs uppercase tracking-[0.18em] text-slate-500">
                      {asset.city ?? 'Unknown city'} · Last enriched{' '}
                      {formatDate(asset.lastEnrichedAt)}
                    </div>
                  </div>
                ))
              ) : (
      ...
      <Card className="space-y-4">
        <div>
          ...
          <h3 className="mt-2 text-xl font-semibold text-white">
            Connector readiness and implementation priority
          </h3>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-400">
            Start with market, macro, and FX APIs if possible. If not, wire the official free public
            stack in order: US FRED, BLS, Treasury Fiscal Data, ECB, then Korea KOSIS. After that,
          ...
          {macroConnectors.map((connector) => (
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <Badge tone={toneForReadiness(connector.status)}>
      ...
      <Card className="space-y-4">
        <div>
          ...
          <h3 className="mt-2 text-xl font-semibold text-white">
            Region sequencing for global underwriting rollout
          </h3>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-400">
          ...
          {launchPlan.map((region) => (
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <Badge tone={toneForLaunchStatus(region.status)}>
                  {region.status.toLowerCase()}
                </Badge>
              </div>
      ...
      <Card className="space-y-4">
        <div>
          ...
          <h3 className="mt-2 text-xl font-semibold text-white">
            Normalized registry for public Korean property and planning data
          </h3>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-400">
            These adapters stay env-configured and fallback-safe. They are intended to feed macro,
            market, parcel, permit, land, and building evidence into the same review-gated
            underwriting workflow rather than a separate research product.
          </p>
        </div>
        <div className="grid gap-4">
      ...
      <Card className="space-y-4">
        <div>
          ...
          <h3 className="mt-2 text-xl font-semibold text-white">
            Official free sources you can actually wire today
          </h3>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-400">
            Free macro data is not the same thing as true realtime data. Most official sources are
            release-based or near-real-time, so the underwriting stack should classify cadence
            explicitly instead of pretending every feed is tick-level live.
          </p>
        </div>
        <div className="grid gap-4">
          {freeMacroCatalog.map((source) => (
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <Badge tone={toneForRealtimeClass(source.realtimeClass)}>
                  {source.realtimeClass.toLowerCase()}
                </Badge>
              </div>
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                <div>
                  ...
                  <div className="mt-2 text-sm text-slate-400">
                    {source.cadence.replaceAll('_', ' ')}
                  </div>
                </div>
                <div>
                  <div className="fine-print">Auth</div>
                  <div className="mt-2 text-sm text-slate-400">
                    {source.auth.replaceAll('_', ' ')}
                  </div>
                </div>
                <div>
