...
export default async function DealsPage({ searchParams }: Props) {
  ...
  const grantedDealIds = await listGrantedScopeIdsForUser(
    actor?.userId,
  ...
  const blockedDeals = scopedDeals.filter((deal) =>
  ...
  const visibleDeals = scopedDeals
    ...
    .sort((left, right) => {
      ...
      const leftDue =
        leftSnapshot?.nextTask?.dueDate?.getTime() ??
        left.deal.nextActionAt?.getTime() ??
        Number.MAX_SAFE_INTEGER;
      const rightDue =
        rightSnapshot?.nextTask?.dueDate?.getTime() ??
        right.deal.nextActionAt?.getTime() ??
        Number.MAX_SAFE_INTEGER;
      return (
        leftDue - rightDue ||
      ...
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <Card className="hero-mesh">
          ...
          <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-400">
            The deal pipeline keeps next action, counterparties, diligence tasks, and risk flags in
            one operator view. This surface is meant for actual processes, not report generation.
          </p>
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            <div className="metric-card">
              <div className="fine-print">Urgent Deals</div>
              <div className="mt-3 text-4xl font-semibold text-white">
                {formatNumber(urgentDeals.length, 0)}
              </div>
              <p className="mt-2 text-sm text-slate-400">
                At least one urgent or high-priority task is still open.
              </p>
            </div>
            <div className="metric-card">
              <div className="fine-print">Blocked Deals</div>
              <div className="mt-3 text-4xl font-semibold text-white">
                {formatNumber(blockedDeals.length, 0)}
              </div>
              <p className="mt-2 text-sm text-slate-400">
                Open execution blockers or unresolved risk flags.
              </p>
            </div>
            <div className="metric-card">
              ...
              <div className="mt-3 text-4xl font-semibold text-white">
                {formatNumber(
                  scopedDeals.filter((deal) => deal.stage === DealStage.CLOSING).length,
                  0
                )}
              </div>
              <p className="mt-2 text-sm text-slate-400">
                Deals already in document, funds flow, or sign-close mode.
              </p>
            </div>
            <div className="metric-card">
              <div className="fine-print">Direct / Proprietary</div>
              <div className="mt-3 text-4xl font-semibold text-white">
                {formatNumber(directDeals.length, 0)}
              </div>
              <p className="mt-2 text-sm text-slate-400">
                Deals sourced away from a broad brokered process.
              </p>
            </div>
            <div className="metric-card">
              <div className="fine-print">Live Exclusivity</div>
              <div className="mt-3 text-4xl font-semibold text-white">
                {formatNumber(liveExclusivityDeals.length, 0)}
              </div>
              <p className="mt-2 text-sm text-slate-400">
                Processes protected by a live exclusivity clock.
              </p>
            </div>
          </div>
        </Card>
            ...
            >
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="max-w-3xl">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <Badge tone={getDealStageTone(deal.stage)}>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <Badge
                        tone={snapshot && snapshot.checklistCompletionPct < 100 ? 'warn' : 'good'}
                      >
                        checklist{' '}
                        {snapshot ? `${formatNumber(snapshot.checklistCompletionPct, 0)}%` : 'N/A'}
                      </Badge>
                      <Badge
                        tone={
                      ...
                      {snapshot?.dueSoonTaskCount ? (
                        <Badge tone="warn">{snapshot.dueSoonTaskCount} due soon</Badge>
                      ) : null}
                      ...
                      <Badge
                        tone={origination.exclusivityLabel.startsWith('Live') ? 'good' : 'neutral'}
                      >
                    ...
                    <p className="mt-4 text-sm leading-7 text-slate-300">
                      {deal.nextAction ??
                        deal.headline ??
                        'No next action yet. Open the deal and set the immediate move.'}
                    </p>
                    {snapshot ? (
                      <p className="mt-3 text-sm text-slate-500">{snapshot.reminderSummary}</p>
                    ) : null}
                    {researchCoverage ? (
                      <p className="mt-2 text-sm text-slate-500">{researchCoverage.headline}</p>
                    ) : null}
                  ...
                  <div className="grid gap-3 text-right md:grid-cols-2 md:text-left xl:grid-cols-4">
                    ...
                    <div>
                      ...
                      <div className="mt-2 text-sm text-white">
                        {latestLenderQuote
                          ? formatCurrency(latestLenderQuote.amountKrw)
                          : 'No lender quote'}
                      </div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className="mt-2 text-sm text-white">
                        ...
                        {formatNumber(readiness.blockerCount, 0)} blockers
                      </div>
                    </div>
                    <div>
              ...
              <div className="mt-5 grid gap-4 border-t border-white/10 pt-5 md:grid-cols-[1fr_auto]">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {deal.counterparties.length > 3 ? (
                    <Badge>+{deal.counterparties.length - 3}</Badge>
                  ) : null}
                  {latestBid ? <Badge>{latestBid.status.toLowerCase()}</Badge> : null}
                  {snapshot?.exclusivityExpiresSoon ? (
                    <Badge tone="warn">exclusivity expiring</Badge>
                  ) : null}
        ...
        {visibleDeals.length === 0 ? (
          <Card>
            ...
            <h2 className="mt-3 text-2xl font-semibold text-white">
              Open the first execution record
            </h2>
