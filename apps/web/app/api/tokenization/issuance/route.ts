...
export async function GET(request: Request) {
  ...
  try {
    ...
    return genericErrorResponse(error, {
      status: 400,
      message: 'Read failed',
      context: { route: 'tokenization/issuance', op: 'GET' }
    });
  }
}
...
export async function POST(request: Request) {
  ...
  if (!hasRequiredAdminRole(actor.role, 'ADMIN')) {
    ...
    // (`getRequiredAdminRoleForPath` → ADMIN). mint/burn/forceTransfer/pause are
    // the highest-consequence on-chain writes; never let them drop to "any active
    // seat".
    return NextResponse.json(
  ...
  try {
    ...
    return genericErrorResponse(error, {
      ...
      context: { route: 'tokenization/issuance', op: 'POST' }
    });
  }
}
