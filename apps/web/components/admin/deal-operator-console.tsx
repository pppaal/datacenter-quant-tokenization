...
function getMatchSuggestion(value: unknown): {
  documentId: string;
...
export function DealOperatorConsole({ deal, snapshot, origination }: Props) {
  ...
  const [sellerGuidanceKrw, setSellerGuidanceKrw] = useState(
    deal.sellerGuidanceKrw != null ? String(deal.sellerGuidanceKrw) : ''
  );
  const [bidGuidanceKrw, setBidGuidanceKrw] = useState(
    deal.bidGuidanceKrw != null ? String(deal.bidGuidanceKrw) : ''
  );
  ...
  const [counterpartyCoverageStatus, setCounterpartyCoverageStatus] =
    useState<RelationshipCoverageStatus>(RelationshipCoverageStatus.PASSIVE);
  ...
  const [lenderStatus, setLenderStatus] = useState<DealLenderQuoteStatus>(
    DealLenderQuoteStatus.INDICATED
  );
  ...
  const [negotiationEventType, setNegotiationEventType] = useState<DealNegotiationEventType>(
    DealNegotiationEventType.SELLER_COUNTER
  );
  ...
  const [closeLossReason, setCloseLossReason] = useState<string>(
    deal.lossReason ?? DealLossReason.OTHER
  );
  ...
  const liveBidRevisions = deal.bidRevisions.filter(
    (bidRevision) =>
      bidRevision.status !== DealBidStatus.DECLINED &&
      bidRevision.status !== DealBidStatus.WITHDRAWN
  );
      ...
      <div className="space-y-6">
        <Card className="space-y-5" data-testid="deal-archive-section">
            ...
            {snapshot.stageTrack.map((item) => (
              ...
              >
                <div className="fine-print">
                  {item.isCurrent ? 'Current' : item.isCompleted ? 'Done' : 'Next'}
                </div>
          ...
          <div className="rounded-[24px] border border-white/10 bg-white/[0.03] p-5">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <Badge
                tone={
                  origination.band === 'HIGH'
                    ? 'good'
                    : origination.band === 'MEDIUM'
                      ? 'warn'
                      : 'danger'
                }
              >
            ...
            <div className="grid gap-4 md:grid-cols-[1fr_2fr_auto]">
              ...
              <label className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Why the stage changed now?"
                />
              </label>
          ...
          >
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <label className="space-y-2">
                ...
                <Input
                  type="number"
                  step="any"
                  value={sellerGuidanceKrw}
                  onChange={(event) => setSellerGuidanceKrw(event.target.value)}
                />
              </label>
              <label className="space-y-2">
                ...
                <Input
                  type="number"
                  step="any"
                  value={bidGuidanceKrw}
                  onChange={(event) => setBidGuidanceKrw(event.target.value)}
                />
              </label>
            </div>
            <div className="grid gap-4 md:grid-cols-[220px_1fr]">
              <label className="space-y-2">
                ...
                <Select
                  value={originationSource}
                  onChange={(event) => setOriginationSource(event.target.value)}
                >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            void run(task.id, async () => {
                              await request(`/api/deals/${deal.id}/tasks/${task.id}`, 'PATCH', {
                                status: value
                              });
                            });
                          }}
          ...
          >
            <div className="grid gap-4 md:grid-cols-[1.2fr_0.8fr]">
              ...
              <Input
                ...
                placeholder="Category (leases, title, utility, etc.)"
              />
            </div>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <Select
                value={requestCounterpartyId}
                onChange={(event) => setRequestCounterpartyId(event.target.value)}
              >
            ...
            <Textarea
              ...
              placeholder="Why this item matters, who owes it, and how it will be used."
            />
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Badge
                          tone={
                            requestItem.priority === TaskPriority.URGENT ||
                            requestItem.priority === TaskPriority.HIGH
                              ? 'warn'
                              : 'neutral'
                          }
                        >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            void run(`${requestItem.id}-suggestion`, async () => {
                              await request(
                                `/api/deals/${deal.id}/document-requests/${requestItem.id}`,
                                'PATCH',
                                {
                                  status: 'RECEIVED',
        ...
        <Card className="space-y-5">
          ...
          >
            ...
            <Textarea
              ...
              value={riskDetail}
              onChange={(event) => setRiskDetail(event.target.value)}
              placeholder="What blocks signing or closing?"
            />
            <div className="flex justify-end">
        ...
        <Card className="space-y-5">
            ...
            onSubmit={(event) => {
              ...
              void run('bid-create', async () => {
                await request(`/api/deals/${deal.id}/bids`, 'POST', {
                  ...
                  closeTimelineDays:
                    bidCloseTimelineDays === '' ? null : Number(bidCloseTimelineDays),
          ...
          >
            <div className="grid gap-4 md:grid-cols-[1fr_220px_220px]">
              <Input
                value={bidLabel}
                onChange={(event) => setBidLabel(event.target.value)}
                placeholder="Label (Initial IOI, Revised LOI, BAFO...)"
              />
              <Select
                value={bidCounterpartyId}
                onChange={(event) => setBidCounterpartyId(event.target.value)}
              >
              ...
              <Select
                value={bidStatus}
                onChange={(event) => setBidStatus(event.target.value as DealBidStatus)}
              >
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
              <Input
                ...
                step="any"
                value={bidPriceKrw}
                onChange={(event) => setBidPriceKrw(event.target.value)}
                placeholder="Bid price KRW"
              />
              <Input
                type="number"
                step="any"
                value={bidDepositKrw}
                onChange={(event) => setBidDepositKrw(event.target.value)}
                placeholder="Deposit KRW"
              />
              <Input
                type="number"
                value={bidExclusivityDays}
                onChange={(event) => setBidExclusivityDays(event.target.value)}
                placeholder="Exclusivity days"
              />
              <Input
                type="number"
                value={bidDiligenceDays}
                onChange={(event) => setBidDiligenceDays(event.target.value)}
                placeholder="DD days"
              />
              <Input
                type="number"
                value={bidCloseTimelineDays}
                onChange={(event) => setBidCloseTimelineDays(event.target.value)}
            ...
            <div className="grid gap-4 md:grid-cols-[220px_1fr]">
              <Input
                type="date"
                value={bidSubmittedAt}
                onChange={(event) => setBidSubmittedAt(event.target.value)}
              />
              <Textarea
                className="min-h-[90px]"
                value={bidNotes}
                onChange={(event) => setBidNotes(event.target.value)}
                placeholder="Commercial changes, certainty points, seller feedback, or BAFO context."
              />
            </div>
            <div className="flex justify-end">
              <Button
                type="submit"
                disabled={
                  busy === 'bid-create' || bidLabel.trim() === '' || bidPriceKrw.trim() === ''
                }
              >
            ...
            {deal.bidRevisions.map((bidRevision) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="mt-2 text-sm text-slate-500">
                      {bidRevision.exclusivityDays
                        ...
                        : 'No exclusivity term'}
                      {' / '}
                      {bidRevision.diligenceDays
                        ? `${bidRevision.diligenceDays}d DD`
                        : 'No DD clock'}
                      {' / '}
                      {bidRevision.closeTimelineDays
                        ? `${bidRevision.closeTimelineDays}d close`
                        : 'No close timeline'}
                    </div>
                    {bidRevision.notes ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Badge tone={getDealBidStatusTone(bidRevision.status)}>
                      {toSentenceCase(bidRevision.status)}
                    </Badge>
                  </div>
                </div>
                <div className="mt-4 flex flex-wrap justify-end gap-2">
                  {dealBidStatusOptions
                    .filter((value) => value !== bidRevision.status)
                    .map((value) => (
                      <Button
        ...
        <Card className="space-y-5">
          ...
          >
            <div className="grid gap-4 md:grid-cols-[1fr_220px_220px]">
              <Input
                value={lenderFacilityLabel}
                onChange={(event) => setLenderFacilityLabel(event.target.value)}
                placeholder="Facility label (Senior term loan, bridge, etc.)"
              />
              <Select
                value={lenderCounterpartyId}
                onChange={(event) => setLenderCounterpartyId(event.target.value)}
              >
                <option value="">
                  {lenderCounterparties.length === 0
                    ? 'Add lender counterparty first'
                    : 'Select lender'}
                </option>
              ...
              <Select
                value={lenderStatus}
                onChange={(event) => setLenderStatus(event.target.value as DealLenderQuoteStatus)}
              >
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
              <Input
                type="number"
                step="any"
                value={lenderAmountKrw}
                onChange={(event) => setLenderAmountKrw(event.target.value)}
              ...
              <Input
                ...
                step="any"
                value={lenderLtvPct}
                onChange={(event) => setLenderLtvPct(event.target.value)}
                placeholder="LTV %"
              />
              <Input
                type="number"
                ...
                onChange={(event) => setLenderSpreadBps(event.target.value)}
                placeholder="Spread bps"
              />
              <Input
                type="number"
                step="any"
                value={lenderAllInRatePct}
                onChange={(event) => setLenderAllInRatePct(event.target.value)}
                placeholder="All-in rate %"
              />
              <Input
                type="number"
                step="any"
                value={lenderDscrFloor}
                onChange={(event) => setLenderDscrFloor(event.target.value)}
                placeholder="DSCR floor"
              />
            </div>
            <div className="grid gap-4 md:grid-cols-3 xl:grid-cols-4">
              <Input
                type="number"
                value={lenderTermMonths}
                onChange={(event) => setLenderTermMonths(event.target.value)}
                placeholder="Term months"
              />
              <Input
                type="number"
                value={lenderIoMonths}
                onChange={(event) => setLenderIoMonths(event.target.value)}
                placeholder="IO months"
              />
              <Input
                type="date"
                value={lenderQuotedAt}
                onChange={(event) => setLenderQuotedAt(event.target.value)}
              />
            </div>
            <Textarea
              className="min-h-[90px]"
              ...
              placeholder="Term sheet notes, hold points, conditions, covenant comments, or process blockers."
            />
            <div className="flex justify-end">
              <Button
                type="submit"
                disabled={
                  busy === 'lender-create' ||
                  lenderFacilityLabel.trim() === '' ||
                  lenderAmountKrw.trim() === ''
                }
              >
            ...
            {deal.lenderQuotes.map((lenderQuote) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="mt-2 text-sm text-slate-500">
                      {lenderQuote.spreadBps != null
                        ...
                        : 'No spread'}
                      {' / '}
                      {lenderQuote.allInRatePct != null
                        ? `${formatNumber(lenderQuote.allInRatePct, 2)}% all-in`
                        : 'No all-in rate'}
                      {' / '}
                      {lenderQuote.dscrFloor != null
                    ...
                    <div className="mt-2 text-sm text-slate-500">
                      {lenderQuote.termMonths != null
                        ...
                        : 'No term'}
                      {' / '}
                      {lenderQuote.ioMonths != null
                        ? `${formatNumber(lenderQuote.ioMonths, 0)}m IO`
                        : 'No IO period'}
                    </div>
                    {lenderQuote.notes ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Badge tone={getDealLenderQuoteStatusTone(lenderQuote.status)}>
                      {toSentenceCase(lenderQuote.status)}
                    </Badge>
                  </div>
                </div>
                <div className="mt-4 flex flex-wrap justify-end gap-2">
                  {dealLenderQuoteStatusOptions
                    .filter((value) => value !== lenderQuote.status)
                    .map((value) => (
                      <Button
                        ...
                        onClick={() => {
                          void run(`${lenderQuote.id}-${value}`, async () => {
                            await request(
                              `/api/deals/${deal.id}/lender-quotes/${lenderQuote.id}`,
                              'PATCH',
                              {
                                status: value
                              }
        ...
        <Card className="space-y-5">
          <div className="flex items-center justify-between gap-4">
            ...
            <Badge tone={liveNegotiationEvents.length > 0 ? 'warn' : 'neutral'}>
              {deal.negotiationEvents.length > 0
                ? `${deal.negotiationEvents.length} tracked`
                : 'empty'}
            </Badge>
          </div>
          <form
          ...
          >
            <div className="grid gap-4 md:grid-cols-[220px_220px_1fr]">
              <Select
                value={negotiationEventType}
                onChange={(event) =>
                  setNegotiationEventType(event.target.value as DealNegotiationEventType)
                }
              >
              ...
              <Select
                value={negotiationCounterpartyId}
                onChange={(event) => setNegotiationCounterpartyId(event.target.value)}
              >
                <option value="">Select counterparty</option>
                {deal.counterparties.map((counterparty) => (
                  <option key={counterparty.id} value={counterparty.id}>
              ...
              <Input
                ...
                onChange={(event) => setNegotiationTitle(event.target.value)}
                placeholder="Title (Seller countered on price, exclusivity granted, etc.)"
              />
            </div>
            <div className="grid gap-4 md:grid-cols-[220px_220px_1fr]">
              <Input
                type="date"
                value={negotiationEffectiveAt}
                onChange={(event) => setNegotiationEffectiveAt(event.target.value)}
              />
              <Input
                type="date"
                value={negotiationExpiresAt}
                onChange={(event) => setNegotiationExpiresAt(event.target.value)}
              />
              <Select
                value={negotiationBidRevisionId}
                onChange={(event) => setNegotiationBidRevisionId(event.target.value)}
              >
            ...
            <Textarea
              ...
              placeholder="What changed in the process, how the seller reacted, what feedback came back, and when the exclusivity clock ends."
            />
            <div className="flex justify-end">
              <Button
                type="submit"
                disabled={busy === 'negotiation-create' || negotiationTitle.trim() === ''}
              >
            ...
            {deal.negotiationEvents.map((event) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Badge tone={getDealNegotiationEventTone(event.eventType)}>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        void run(`${event.id}-extend`, async () => {
                          await request(
                            `/api/deals/${deal.id}/negotiation-events/${event.id}`,
                            'PATCH',
                            {
      ...
      <div className="space-y-6">
          ...
          >
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Select
                ...
                onChange={(event) => setCounterpartyRole(event.target.value)}
              >
              ...
              <Input
                ...
                onChange={(event) => setCounterpartyCompany(event.target.value)}
                placeholder="Company"
              />
              <Input
              ...
              />
              <Input
                value={counterpartyCoverageOwner}
                onChange={(event) => setCounterpartyCoverageOwner(event.target.value)}
                placeholder="Coverage owner"
              />
              <Select
                value={counterpartyCoverageStatus}
                onChange={(event) =>
                  setCounterpartyCoverageStatus(event.target.value as RelationshipCoverageStatus)
                }
              >
              ...
              <Input
                type="date"
                value={counterpartyLastContactAt}
                onChange={(event) => setCounterpartyLastContactAt(event.target.value)}
              />
              <Input
                value={counterpartyNotes}
                onChange={(event) => setCounterpartyNotes(event.target.value)}
                ...
                <form
                  ...
                  onSubmit={(event) => {
                    ...
                    void run(
                      ...
                      async () => {
                        await request(
                          `/api/deals/${deal.id}/counterparties/${counterparty.id}`,
                          'PATCH',
                          {
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {counterparty.coverageOwner ? (
                        <Badge tone="neutral">{counterparty.coverageOwner}</Badge>
                      ) : null}
                    </div>
                  </div>
                  <div className="grid gap-4 md:grid-cols-3">
                    <Input
                      name="company"
                      defaultValue={counterparty.company ?? ''}
                      placeholder="Company"
                    />
                    <Input
                      name="email"
                    ...
                    <Input
                      name="coverageOwner"
                      defaultValue={counterparty.coverageOwner ?? ''}
                      placeholder="Coverage owner"
                    />
                    ...
                    <Input
                      name="lastContactAt"
                      type="date"
                      defaultValue={toDateValue(counterparty.lastContactAt)}
                    />
                  </div>
                  <Textarea
                    name="notes"
                  ...
                  <div className="flex items-center justify-between gap-4 text-sm text-slate-400">
                    <div>
                      ...
                      {counterparty.lastContactAt
                        ? formatDate(counterparty.lastContactAt)
                        : 'not logged'}
                    </div>
                    <Button
                      type="submit"
                      variant="secondary"
                      disabled={busy === `counterparty-${counterparty.id}`}
                    >
        ...
        <Card className="space-y-5">
          <div className="flex items-center justify-between gap-4">
            ...
            <Badge
              tone={
                snapshot.notesByRole.reduce((count, item) => count + item.notes.length, 0) > 0
                  ? 'good'
                  : 'neutral'
              }
            >
          ...
          >
            <div className="grid gap-4 md:grid-cols-[170px_1fr_1fr]">
              <Select
                value={noteRole}
                onChange={(event) =>
                  setNoteRole(event.target.value as 'BROKER' | 'SELLER' | 'BUYER')
                }
              >
              ...
              <Select
                value={noteCounterpartyId}
                onChange={(event) => setNoteCounterpartyId(event.target.value)}
                disabled={noteCounterparties.length === 0}
              >
                <option value="">
                  {noteCounterparties.length === 0
                    ? 'Add counterparty first'
                    : 'Select counterparty'}
                </option>
            ...
            <Textarea
              ...
              placeholder="What did they say and why does it matter?"
            />
            <div className="flex justify-end">
              <Button
                type="submit"
                disabled={busy === 'note-create' || noteCounterparties.length === 0}
              >
          ...
          >
            <div className="grid gap-4 md:grid-cols-[220px_220px_1fr]">
              <Select
                value={closeOutcome}
                onChange={(event) =>
                  setCloseOutcome(event.target.value as 'CLOSED_WON' | 'CLOSED_LOST')
                }
              >
              ...
              <Select
                ...
                onChange={(event) => setCloseLossReason(event.target.value)}
              >
              ...
              <Textarea
                ...
                placeholder="Write the final outcome, economics, and what matters for handoff or post-mortem."
              />
            </div>
            <div className="flex items-center justify-between gap-4">
              <Button
                ...
                data-testid="deal-archive-button"
                onClick={() => {
                  void run(
                    'archive',
                    async () => {
