      ...
      {data.regimeTransition?.hasTransition ? (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <Badge
              tone={
                data.regimeTransition.overallDirection === 'TIGHTENING'
                  ? 'danger'
                  : data.regimeTransition.overallDirection === 'EASING'
                    ? 'good'
                    : 'warn'
              }
            >
        ...
        <div className="rounded-[22px] border border-white/10 bg-white/[0.03] p-6">
          ...
          {data.narrative.riskCallout ? (
            <div className="mt-4 rounded-[14px] border border-orange-500/20 bg-orange-500/5 px-4 py-3">
              ...
              <p className="mt-1 text-sm leading-6 text-orange-300/80">
                {data.narrative.riskCallout}
              </p>
            </div>
          ) : null}
          {data.narrative.cached ? <Badge>cached</Badge> : null}
        </div>
      ) : null}
