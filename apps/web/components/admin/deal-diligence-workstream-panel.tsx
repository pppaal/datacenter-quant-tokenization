...
export function DealDiligenceWorkstreamPanel({
  dealId,
  stageLabel,
  workstreams,
  availableDocuments
  ...
  const [newType, setNewType] = useState<DealDiligenceWorkstreamType>(
    DealDiligenceWorkstreamType.LEGAL
  );
  const [newStatus, setNewStatus] = useState<DealDiligenceWorkstreamStatus>(
    DealDiligenceWorkstreamStatus.IN_PROGRESS
  );
    ...
    <Card className="space-y-5">
      ...
      >
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <Select
            value={newType}
            onChange={(event) => setNewType(event.target.value as DealDiligenceWorkstreamType)}
          >
          ...
          <Select
            value={newStatus}
            onChange={(event) => setNewStatus(event.target.value as DealDiligenceWorkstreamStatus)}
          >
          ...
          <Input
            value={newAdvisor}
            onChange={(event) => setNewAdvisor(event.target.value)}
            placeholder="External advisor"
          />
          <Input
            value={newReportTitle}
            onChange={(event) => setNewReportTitle(event.target.value)}
            placeholder="Report or memo title"
          />
          <Input
            type="date"
            value={newDueDate}
        ...
        <Textarea
          ...
          placeholder="Current diligence scope and status"
        />
        <Textarea
          className="min-h-[88px]"
          value={newBlocker}
          onChange={(event) => setNewBlocker(event.target.value)}
          placeholder="Current blocker, if any"
        />
        <Textarea
          className="min-h-[88px]"
          value={newNotes}
          ...
          <form
            ...
            onSubmit={(event) => {
              ...
              void run(
                ...
                async () => {
                  await send(
                    `/api/deals/${dealId}/diligence-workstreams/${workstream.id}`,
                    'PATCH',
                    {
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Badge tone={getStatusTone(workstream.status)}>
                  {toSentenceCase(workstream.status)}
                </Badge>
                {workstream.signedOffAt ? (
                  <Badge tone="good">signed {formatDate(workstream.signedOffAt)}</Badge>
                ) : null}
              </div>
            </div>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <Input
                name="ownerLabel"
                defaultValue={workstream.ownerLabel ?? ''}
                placeholder="Internal owner"
              />
              <Input
                name="advisorName"
                defaultValue={workstream.advisorName ?? ''}
                placeholder="External advisor"
              />
              <Input
                name="reportTitle"
                defaultValue={workstream.reportTitle ?? ''}
            ...
            <Textarea
              name="summary"
              className="min-h-[88px]"
              defaultValue={workstream.summary ?? ''}
              placeholder="Current scope and status"
            />
            <Textarea
              name="blockerSummary"
              className="min-h-[88px]"
            ...
            <div className="space-y-3 rounded-[18px] border border-white/10 bg-slate-950/35 p-4">
                ...
                <Button
                  ...
                  onClick={(event) => {
                    ...
                    const documentSelect = form?.querySelector<HTMLSelectElement>(
                      `select[name="deliverable-${workstream.id}"]`
                    );
                    const noteInput = form?.querySelector<HTMLInputElement>(
                      `input[name="deliverable-note-${workstream.id}"]`
                    );
                    ...
                    void run(
                      ...
                      async () => {
                        await send(
                          `/api/deals/${dealId}/diligence-workstreams/${workstream.id}/deliverables`,
                          'POST',
                          {
                            documentId,
                            note: note || null
              ...
              <div className="grid gap-3 rounded-[16px] border border-white/10 bg-black/20 p-3 lg:grid-cols-[1.2fr_0.9fr_1fr_1fr_auto]">
                ...
                <Button
                  ...
                  onClick={(event) => {
                    ...
                    const titleInput = form?.querySelector<HTMLInputElement>(
                      `input[name="upload-title-${workstream.id}"]`
                    );
                    const typeSelect = form?.querySelector<HTMLSelectElement>(
                      `select[name="upload-type-${workstream.id}"]`
                    );
                    const noteInput = form?.querySelector<HTMLInputElement>(
                      `input[name="upload-note-${workstream.id}"]`
                    );
                    const fileInput = form?.querySelector<HTMLInputElement>(
                      `input[name="upload-file-${workstream.id}"]`
                    );
                ...
                >
                  {busy === `upload-deliverable-${workstream.id}`
                    ? 'Uploading...'
                    : 'Upload + Link'}
                </Button>
              </div>
