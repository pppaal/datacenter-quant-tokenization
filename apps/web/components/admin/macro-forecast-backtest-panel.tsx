    ...
    <Card>
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold text-white">One-step value prediction check</h2>
          <p className="mt-2 max-w-2xl text-sm leading-7 text-slate-400">
            Uses a simple momentum forecast on macro factor values and compares the predicted next reading against the realized next reading. This is the first numeric forecast validation layer.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-4">
        {[
          ['Markets', formatNumber(backtest.summary.marketCoverage, 0), 'Markets with forecast samples'],
          ['Samples', formatNumber(backtest.summary.sampleCount, 0), 'Predicted-next vs actual-next checks'],
          ['Direction', formatNumber(backtest.summary.directionalHitRatePct, 1) + '%', 'How often the move direction matched'],
          ['Mean Error', formatNumber(backtest.summary.meanAbsoluteErrorPct, 1) + '%', 'Average normalized next-value error']
            ...
            <div key={market.market} className="rounded-[24px] border border-white/10 bg-white/[0.03] p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 text-xs uppercase tracking-[0.18em] text-slate-500">
                    {market.latestActualDate ? `latest ${formatDate(market.latestActualDate)}` : 'history unavailable'}
                  </div>
                </div>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-[160px_1fr_1fr]">
                <div className="rounded-[20px] border border-white/10 bg-slate-950/35 p-4">
                  ...
                  <p className="mt-2 text-xs text-slate-500">{formatNumber(market.sampleCount, 0)} forecast samples</p>
                </div>
                <div className="rounded-[20px] border border-white/10 bg-slate-950/35 p-4">
                  <div className="fine-print">Strongest Factor</div>
                  <div className="mt-3 text-base font-semibold text-white">{market.strongestFactor?.label ?? 'N/A'}</div>
