...
import { buildForecastDecisionNarrative, type ForecastDecisionGuide } from '@/lib/services/forecast/decision';
    ...
    <Card>
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-7 text-slate-400">
            Lightweight gradient boosting on historical valuation paths. This is a learned forecast layer, separate from the deterministic scenario and Monte Carlo screens.
          </p>
        </div>
      ...
      {readingGuide ? (
        <div className="mt-5 rounded-[24px] border border-accent/20 bg-accent/10 p-5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {readingGuide.challengerModelKey ? <Badge tone="neutral">{readingGuide.challengerModelKey}</Badge> : null}
          </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-4">
        <div className="rounded-[24px] border border-white/10 bg-white/[0.03] p-5">
          ...
          <div className="mt-3 text-3xl font-semibold text-white">{formatNumber(forecast.sampleCount, 0)}</div>
          <p className="mt-2 text-sm text-slate-400">{formatNumber(forecast.assetCoverage, 0)} assets with sequential history.</p>
        </div>
        <div className="rounded-[24px] border border-white/10 bg-white/[0.03] p-5">
          <div className="fine-print">Value Drift</div>
          <div className="mt-3 text-3xl font-semibold text-white">
            {forecast.predictedValueChangePct === null ? 'N/A' : formatPercent(forecast.predictedValueChangePct)}
          </div>
          <p className="mt-2 text-sm text-slate-400">{forecast.forecastHorizonMonths}-month predicted change.</p>
        </div>
        <div className="rounded-[24px] border border-white/10 bg-white/[0.03] p-5">
          ...
          <div className="mt-3 text-3xl font-semibold text-white">
            {forecast.predictedValueKrw === null
              ? 'N/A'
              : formatCurrencyFromKrwAtRate(forecast.predictedValueKrw, displayCurrency, fxRateToKrw)}
          </div>
        ...
        <div className="rounded-[24px] border border-white/10 bg-white/[0.03] p-5">
          ...
          <div className="mt-3 text-3xl font-semibold text-white">
            {forecast.predictedDscr === null ? 'N/A' : `${formatNumber(forecast.predictedDscr, 2)}x`}
          </div>
          <p className="mt-2 text-sm text-slate-400">
            {forecast.predictedDscrChangePct === null ? 'Not enough data.' : `${formatPercent(forecast.predictedDscrChangePct)} drift`}
          </p>
        </div>
      </div>
          ...
          <div className="mt-3 space-y-2">
            {forecast.topDrivers.length > 0 ? (
              forecast.topDrivers.map((driver) => (
                <div key={driver.featureKey} className="flex items-center justify-between gap-3 text-sm text-slate-300">
              ...
              <div className="text-sm text-slate-500">More sequential valuation history is needed before feature attribution becomes meaningful.</div>
            )}
          </div>
        </div>
      </div>
