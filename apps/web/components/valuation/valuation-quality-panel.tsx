    ...
    <Card className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-2 text-2xl font-semibold text-white">Coverage, gaps, and source pressure</div>
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_1fr]">
        <div className="rounded-[22px] border border-white/10 bg-white/[0.03] p-4">
          ...
          {summary.missingInputs.length === 0 ? (
            <p className="mt-3 text-sm text-emerald-300">Core lease, comparable, CAPEX, power, permit, and legal inputs are all present.</p>
          ) : (
        ...
        <div className="rounded-[22px] border border-white/10 bg-white/[0.03] p-4">
          ...
          {summary.featureSources.length === 0 ? (
            <p className="mt-3 text-sm text-slate-400">No promoted feature snapshots were attached to this valuation run.</p>
          ) : (
            <div className="mt-3 space-y-3">
              {summary.featureSources.map((source) => (
                <div key={`${source.namespace}-${source.sourceVersion}`} className="rounded-2xl border border-white/10 bg-slate-950/30 p-3">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="text-sm font-semibold text-white">{source.label}</div>
                    <div className="font-mono text-[11px] uppercase tracking-[0.18em] text-slate-500">{source.namespace}</div>
                  </div>
