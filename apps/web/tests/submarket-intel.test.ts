...
const tenant: TenantExposure = {
  tenant: {
    companyId: 'C1',
    companyName: 'Blue Chip',
    industry: 'TECH',
    fiscalYear: 2025,
    isListed: true,
    totalAssetsKrw: 500e12,
    totalLiabilitiesKrw: 100e12,
    currentAssetsKrw: 220e12,
    currentLiabilitiesKrw: 70e12,
    cashAndEquivalentsKrw: 90e12,
    totalDebtKrw: 30e12,
    revenueKrw: 300e12,
    operatingIncomeKrw: 55e12,
    netIncomeKrw: 45e12,
    interestExpenseKrw: 2e12,
    operatingCashFlowKrw: 70e12,
...
const criteria: SponsorCriteria = {
  ...
  minCheckSizeKrw: 20e9,
  maxCheckSizeKrw: 500e9,
  minDiscountPct: 15,
...
const txns: CompTransaction[] = [
  ...
  {
    id: 'T3',
    dealDate: daysAgo(200),
    priceKrw: 80e9,
    gfaSqm: 7000,
    capRatePct: 5.7,
    pricePerSqmKrw: 11.4e6,
    buyerName: null,
    sellerName: null,
    assetLabel: 'C'
  }
...
const pipeline: PipelineDelivery[] = [
  {
    ...
    expectedDeliveryDate: daysAhead(300),
    expectedGfaSqm: 30000,
    developer: null,
...
const competitiveInput: CompetitiveIntelInput = {
  ...
  subject: {
    assetLabel: 'Subject',
    currentCapRatePct: 5.5,
    currentMonthlyRentKrwPerSqm: 68000,
...
test('buildSubmarketIntel: HIGH conviction yields IMMEDIATE bid action', () => {
  ...
  if (intel.conviction.band === 'HIGH') {
    assert.ok(
      intel.playbook.some((a) => a.priority === 'IMMEDIATE' && a.category === 'ORIGINATION')
    );
  }
});
...
test('buildSubmarketIntel: AVOID band emits pause action', () => {
  ...
  const intel = buildSubmarketIntel(
    { conviction: avoidInput, competitive: competitiveInput },
    asOf
  );
